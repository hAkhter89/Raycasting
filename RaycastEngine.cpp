...
// He's also the person who's made a documentation for raycasting, which is
// where I have learnt this from.
...
void RaycastEngine::updateRun(bool run) { isRunning = run; }

bool RaycastEngine::getIsRunning() const { return isRunning; }

void RaycastEngine::addEntity(Entity *entity) { entities.push_back(entity); }

std::vector<Entity*>& RaycastEngine::getEntities(){ return entities; }

void RaycastEngine::render(sf::RenderWindow &window, const Player &player, const Map &map) {
    // Z-buffer stores wall distances for each screen column (for sprite
    // occlusion) Oclusion is when one object hides another. This makes sure not
    // to display any sprites that shouldn't be displayed
    ...
    for (int x = 0; x < screenWidth; ++x) {
        ...
        sf::Vector2f playerDir =
                ::getDir(player); // uses the friend function to access private data
                                                    // elements (with :: global scope)
        sf::Vector2f playerPlane =
                ::getPlane(player); // uses the friend function to access data elements,
                                                        // we couldve also done this with getters
        sf::Vector2f rayDir(playerDir.x + playerPlane.x * cameraX,
        ...
        // DDA (Digital Differential Analysis) algorithm - step through grid until
        // we hit a wall
        bool hit = false;
        while (!hit) {
            ...
            if (map.getGrid()[mapPos.y][mapPos.x] > 0) hit = true;
        }
        ...
        // side note: using actual ray distance to the wall makes the wall look
        // curved (fish eye effect) since we have used planes (not rays themselves),
        // we dont face taht issue

        // Store distance in z-buffer for sprite occlusion
        zBuffer[x] = perpWallDist;

        // Calculate wall height on screen (inversely proportional to distance, the
        // farther the wall the smaller it appears)
        ...
        sf::Color color = sf::Color(46, 46, 46);
        if (side == 1) color = sf::Color(32, 32, 32);
    ...
    for (size_t i = 0; i < entities.size(); ++i) {
        ...
        double dist =
                dx * dx +
                dy * dy; // Squared distance, (faster, good enough for sorting)
    ...
    for (const auto &sprite : sprites) {
        ...
        double spriteX = entity->getPosition().x - player.getPosition().x;
        double spriteY = entity->getPosition().y - player.getPosition().y;

        // Transform sprite position to camera space using inverse camera matrix
        // A lot of big words just to say that it converts coordinates (x, y) into
        // what the camera can actually see The cameras viewing directions becomes
        // the forward direction The camera plane (which is prependicular to viewing
        // direction) becomes the horizontal
        double invDet = 1.0 / (getPlane(player).x * getDir(player).y - getDir(player).x * getPlane(player).y);
        
        // how far to the left or right the sprite is
        double transformX = invDet * (getDir(player).y * spriteX - getDir(player).x * spriteY);                
        // how far away the sprite is
        double transformY =    invDet * (-getPlane(player).y * spriteX + getPlane(player).x * spriteY);     
        ...
        int spriteHeight = static_cast<int>(std::abs(screenHeight / transformY) * entity->getSize());
        ...
        const float radiusNorm = 0.5f;    // Radius in normalized [0,1] texture space (it isnt excatly
                                                                        // normalized right now, but will be when we calculate u)
        ...
        for (int stripe = drawStartX; stripe < drawEndX; ++stripe) {
            ...
            // side note: normalizing creates a coordinate system independent of
            // pixels themselves. suuuuuuuuuuuuuuuuper useful

            // Check if we're inside the circle horizontally
            // This is just optimization, if the stripe we are going to draw is
            // outside the circle bounds, why waste our oh so precious computing power
            double centerU = 0.5;
            double du = u - centerU;
            if (std::fabs(du) > radiusNorm) continue; // Outside circle

            // Calculate vertical extent of circle at this horizontal position (chord
            // length) chord = line connecting two points of a circle So what we are
            // doing is displaying each (vertical) chord. Pythagorean theorem go
            // brrrrrrrrr
            double halfChord = std::sqrt(radiusNorm * radiusNorm - du * du);
            double vStart = 0.5 - halfChord;
            double vEnd = 0.5 + halfChord;

            // side note: the formula [ sqrt(r^2 - du^2) ] actually calculates the
            // height of the circle from the horizontal center this is why its a half
            // chord, subtracting that from vStart and adding that to vEnd gives us
            // the full chord
