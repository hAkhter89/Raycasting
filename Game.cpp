...
Game::Game() :  window(sf::VideoMode(SCREEN_WIDTH, SCREEN_HEIGHT), "Raycasting Game"), map(MAP_WIDTH, MAP_HEIGHT),
    ...
    startButton.setFillColor(sf::Color(32, 32, 32)); // Green
...
Game::~Game() {
    ...
    for (auto entity : raycast.getEntities()) {
        delete entity;
    }
}
    ...
    while (window.isOpen() && raycast.getIsRunning()) {
        ...
        if (currentState == Menu && event.type == sf::Event::MouseButtonPressed) {
            if (event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (startButton.getGlobalBounds().contains(mousePos)) {
                    ...
                    // Reset all enemies to starting positions (so they don't spawn right
                    // next to you)

                    for (auto entity : raycast.getEntities()) {
                        if (entity->getType() == "Seeker") {
                            // Reset to original positions based on entity index
                            if (entity == raycast.getEntities()[0]) {
                                entity->setPosition(sf::Vector2f(8.0f, 8.0f));
                            }
                            else if (entity == raycast.getEntities()[1]) {
        ...
        if (currentState == Menu) {
            ...
            if (player.getHealth() <= 0) {
                ...
                survivalText.setOrigin(survivalRect.left + survivalRect.width / 2.0f, survivalRect.top + survivalRect.height / 2.0f);
            ...
            // Show death screen for 2 seconds then return to menu (so you can try
            // again)
